...
> **세션 시작 시 `HANDOFF.md`를 먼저 읽을 것** — 운영 방침(전권 위임), 자격증명 env 목록,
> 즉시 할 일, 최근 변경 요약. 자격증명은 환경 env vars로 자동 주입됨 (값 출력 금지).
...
### 메시지 포맷 (미미 스타일 — 2026-09-23 사용자 지시 "이정도 수준으로, 깔끔하게")
```
📈 신고가 — 2026.09.23.(수) 16:05 KST
(반도체) 엑시콘, 샘씨엔에스
(기계) 아스플로★
(소형주/1,000억↓) 이스트에이드
★ 역사적 신고가
...
(반도체) 마이크로컨텍솔
```
- **신고가** = 52주 종가 신고가 ∪ **장중 고가 신고가**(`high_52w_hi`, 미미와 동일 정의 —
...
`US_SCREENER_NASDAQ_TIMEOUT_S`(90 — screener 응답 40초+인 날 있음) · `US_SCREENER_ROWS_MAX_AGE_H`(72 — 라이브
실패 시 마지막 정상 screener 응답 디스크 캐시 `us_nasdaq_screener.json` 사용 한도).
