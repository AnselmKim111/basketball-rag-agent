...
report 차트 테마(chart_theme) 재사용. PNG bytes 반환(파일 저장 불필요).
rows: db.load_ohlcv 형식 (open/high/low/close = cents 정수, volume = 주식수).
"""
...
def render_candle_volume(ticker: str, rows: list[dict], title: str | None = None,
    ...
    try:
        ...
        df = pd.DataFrame({
            "Open": [r["open"] / 100 for r in sub],
            "High": [r["high"] / 100 for r in sub],
            "Low": [r["low"] / 100 for r in sub],
            "Close": [r["close"] / 100 for r in sub],
        ...
        log.exception("[us_screener.chart] %s 렌더 실패", ticker)
        return None
