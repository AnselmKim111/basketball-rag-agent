...
DEFAULT_PROMPTS = {
    ...
    "idea_importance": (
        "사용자 투자 아이디어와 1단계 리서치 결과를 받아 importance_score(1-10), verdict, "
        "key_arguments_for/against, investment_implication, proceed_with_top5를 JSON으로 출력하라. "
        "구조적 변화 vs 노이즈를 단호하게 평가하라."
    ),
    "idea_narrow": (
    ...
    "idea_synthesis": (
        ...
        "각 종목별 사업부/폭/기울기 + 1500자 thesis + 정량 수치 + 참고 종목 리포트 파일명만 JSON으로 출력하라."
    ),
    "idea_contrarian": (
        "사용자 thesis가 깨질 시나리오 3개 + 부정 영업레버리지가 세게 걸리는 RISK Top 5를 JSON으로 출력하라. "
        "후보는 입력 candidates 안에서만. 800자 thesis + 정량 수치 + own 리포트 파일명만."
    ),
}
