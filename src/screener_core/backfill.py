...
`make_api(db, data_source, universe)` closure factory로 시장별 모듈 주입.
"""
...
def make_api(db, data_source, universe) -> SimpleNamespace:
    """db/data_source/universe 주입 → backfill closure 반환."""
    ...
    return SimpleNamespace(run_full_backfill=run_full_backfill)
